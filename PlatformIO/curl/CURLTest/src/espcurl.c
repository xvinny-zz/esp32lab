        ...
        if (s->ptr)
        {
            if ((s->status == 0) && ((size * nmemb) > 0))
            {
                for (int i = 0; i < (size * nmemb); i++)
                {
                    if (s->len < (s->maxlen - 2))
                    {
                        if (((buf[i] == 0x0a) || (buf[i] == 0x0d)) || ((buf[i] >= 0x20) && (buf[i] >= 0x20)))
                            s->ptr[s->len] = buf[i];
                        else
                            s->ptr[s->len] = '.';
                        s->len++;
...
int Curl_GET(char *url, char *fname, char *hdr, char *body, int hdrlen, int bodylen)
{
    ...
    if (res != CURLE_OK)
    {
        ...
        if (body)
            snprintf(body, bodylen, "%s", curl_easy_strerror(res));
        err = -7;
        goto exit;
    }
    ...
    if (get_data.tofile)
    {
        ...
        {
            if (strcmp(fname, "simulate") == 0)
                snprintf(body, bodylen, "SIMULATED save to file; size=%d", get_data.len);
            else
                snprintf(body, bodylen, "Saved to file %s, size=%d", fname, get_data.len);
        }
    }
...
static int _Curl_FTP(uint8_t upload, char *url, char *user_pass, char *fname, char *hdr, char *body, int hdrlen, int bodylen, uint8_t sftp)
{
    ...
    if (get_data.tofile)
    {
        ...
        if (body)
        {
            ...
            {
                if (strcmp(fname, "simulate") == 0)
                    snprintf(body, bodylen, "SIMULATED upload from file; size=%d", get_data.len);
                else
                    snprintf(body, bodylen, "Uploaded file %s, size=%d", fname, fsize);
            }
            else
            {
                if (strcmp(fname, "simulate") == 0)
                    snprintf(body, bodylen, "SIMULATED download to file; size=%d", get_data.len);
                else
                    snprintf(body, bodylen, "Downloaded to file %s, size=%d", fname, get_data.len);
            }
        }
    }

...
#define UART_GPIO_TX 13
#define UART_GPIO_RX 14
#define UART_BDRATE 9600
...
const char *PPP_User = "tim";
const char *PPP_Pass = "tim";
...
static char * _configAPN;
static char * _configUser;
static char * _configPwd;

static bool _isHardwareSetup = false;
...
static GSM_Cmd cmd_Reg =
{
	...
	.cmdSecondResponseOnOk = "CREG: 0,5",
	.timeoutMs = 3000,
	.delayMs = 2000,
	.skip = 0,
};
...
static void _disconnect(uint8_t rfOff)
{
	...
	while (res == 0) {
		...
		if (n > 10) {
			...
			// O sistema deve reiniciar se n�o consegue desconectar...
			esp_restart(); //TODO: modifica��o feita por Vinicius de S�
		}
		vTaskDelay(100 / portTICK_PERIOD_MS);
		res = atCmd_waitResponse("ATH\r\n", GSM_OK_Str, "NO CARRIER", 5, 3000, NULL, 0);
	}
...
static void pppos_client_task()
{
    ...
    if (_isHardwareSetup == false)
    {
		...
		ESP_LOGI( TAG, "Setting GSM serial %d, GPIO_TX=%d, GPIO_RX=%d, BaudRate=%d...", uart_num, UART_GPIO_TX, UART_GPIO_RX, UART_BDRATE);
		...
		_isHardwareSetup = true;
    }

	// TODO: Vinicius
	//char PPP_ApnATReq[sizeof(CONFIG_GSM_APN)+24];
	char * PPP_ApnATReq = (char *) malloc(strlen(_configAPN) + 24 );
	...
	while(1)
	{
		...
		while(gsmCmdIter < GSM_InitCmdsSize)
		{
			...
			{
				if (GSM_Init[gsmCmdIter]->hasSecondResponse == true)
				{
					#if GSM_DEBUG
					ESP_LOGW( TAG, "Testing second response option..." );
					#endif

					if (atCmd_waitResponse(GSM_Init[gsmCmdIter]->cmd,
															GSM_Init[gsmCmdIter]->cmdSecondResponseOnOk, NULL,
															GSM_Init[gsmCmdIter]->cmdSize,
															GSM_Init[gsmCmdIter]->timeoutMs, NULL, 0) == 0)
					{
						// * No response or not as expected, start from first initialization command
						#if GSM_DEBUG
						ESP_LOGW( TAG, "Wrong second response, restarting..." );
						#endif
		...
		pppapi_set_auth(ppp, PPPAUTHTYPE_PAP, _configUser, _configPwd);
...
int ppposInit()
{
	ESP_LOGI( "PPPOS", "APN: %s | User: %s | Pwd: %s", _configAPN, _configUser, _configPwd );
...
void setPPPosConfig(char * apn, char * user, char * pwd)
{
	_configAPN = apn;
	_configUser = user;
	_configPwd = pwd;
